...
export class App_MessagesRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfAppMessageDto: RequestModel<App_MessagesDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfAppMessageDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfAppMessageDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfAppMessageDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<App_MessagesDto>> {
    try {
  ...
  @Get('/search')
  getAllAppMessagesBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.app_messagesFacade.search(requestModel);

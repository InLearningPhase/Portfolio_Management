...
export class Tenant_User_App_RolesRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfTenantUserAppRolesDto: RequestModel<Tenant_User_App_RolesDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfTenantUserAppRolesDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfTenantUserAppRolesDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfTenantUserAppRolesDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<Tenant_User_App_RolesDto>> {
    try {
  ...
  @Get('/search')
  getAllTenantUserAppRolesBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.tenant_user_app_rolesFacade.search(requestModel);

...
export class Tenant_User_AppsRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfTenantUserAppsDto: RequestModel<Tenant_User_AppsDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfTenantUserAppsDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfTenantUserAppsDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfTenantUserAppsDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<Tenant_User_AppsDto>> {
    try {
  ...
  @Get('/search')
  getAllTenantUserAppsBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.tenant_user_appsFacade.search(requestModel);

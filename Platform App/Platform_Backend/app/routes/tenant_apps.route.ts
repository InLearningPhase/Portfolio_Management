...
export class Tenant_AppsRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfTenantAppsDto: RequestModel<Tenant_AppsDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfTenantAppsDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfTenantAppsDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfTenantAppsDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<Tenant_AppsDto>> {
    try {
  ...
  @Get('/search')
  getAllTenantAppsBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.tenant_appsFacade.search(requestModel);

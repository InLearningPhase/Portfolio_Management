...
export class App_RolesRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfAppRolesDto: RequestModel<App_RolesDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfAppRolesDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfAppRolesDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfAppRolesDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<App_RolesDto>> {
    try {
  ...
  @Get('/search')
  getAllAppRolesBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.app_rolesFacade.search(requestModel);

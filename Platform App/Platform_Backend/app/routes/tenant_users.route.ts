...
export class Tenant_UsersRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfTenantUsersDto: RequestModel<Tenant_UsersDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfTenantUsersDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfTenantUsersDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfTenantUsersDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<Tenant_UsersDto>> {
    try {
  ...
  @Get('/search')
  getAllTenantUsersBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.tenant_usersFacade.search(requestModel);

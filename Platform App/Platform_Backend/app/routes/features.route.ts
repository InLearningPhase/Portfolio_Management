...
export class FeaturesRoutes{
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<FeaturesDto>> {
    try {
  ...
  @Get('/search')
  getAllFeaturesBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.featuresFacade.search(requestModel);

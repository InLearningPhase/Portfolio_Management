...
export class Tenant_User_App_AlertsRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfTenantUserAppAlertDto: RequestModel<Tenant_User_App_AlertsDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfTenantUserAppAlertDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfTenantUserAppAlertDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfTenantUserAppAlertDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<Tenant_User_App_AlertsDto>> {
    try {
  ...
  @Get('/search')
  getAllTenantUserAppAlertsBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.tenant_user_app_alertsFacade.search(requestModel);

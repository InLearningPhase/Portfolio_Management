...
export class Feature_PermissionsRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfFeaturePermissionDto: RequestModel<Feature_PermissionsDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfFeaturePermissionDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfFeaturePermissionDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfFeaturePermissionDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<Feature_PermissionsDto>> {
    try {
  ...
  @Get('/search')
  getAllFeaturePermissionBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.feature_permissionsFacade.search(requestModel);

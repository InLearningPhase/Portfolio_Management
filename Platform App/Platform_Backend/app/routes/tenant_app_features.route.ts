...
export class Tenant_App_FeaturesRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfTenantAppFeaturesDto: RequestModel<Tenant_App_FeaturesDto> = result["message"];
              errorResult.setStatus(new Message("500", error, null))
              errorResult.setSocketId(requestModelOfTenantAppFeaturesDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfTenantAppFeaturesDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfTenantAppFeaturesDto.RequestGuid);
  ...
  @Get(':id')
  getAllProductsByIds(@Param('id') id: number): Promise<ResponseModel<Tenant_App_FeaturesDto>> {
    try {
  ...
  @Get('/search')
  getAllTenantAppFeaturesBySearch(@Req() req: Request) {
    try {

      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
      console.log(requestModel)

      return this.tenant_app_featuresFacade.search(requestModel);

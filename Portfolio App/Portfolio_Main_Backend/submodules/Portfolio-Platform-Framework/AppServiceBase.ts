...
import { ServiceOperationResultType } from "../Portfolio-Platform-Common/ServiceOperationResultType";
import { Brackets, createConnection, EntitySchema, getRepository, ObjectType, Repository, SelectQueryBuilder } from "typeorm";
import { DtoBase } from "../Portfolio-Main-Dtos/DtoBase/DtoBase";
...
import { EntityBase } from "../Portfolio-Main-Entities/EntityBase/EntityBase";
import { SNS_SQS } from "../Portfolio-Main-RabbitMQConfig/SNS_SQS";
import { Message } from "submodules/Portfolio-Platform-Common/Message";
import { ConditionalOperation } from "submodules/Portfolio-Platform-Common/conditionOperation";
import { RequestModelQuery } from "submodules/Portfolio-Platform-Common/RequestModelQuery";
import { Condition } from "submodules/Portfolio-Platform-Common/condition";
...
export default class AppService<TEntity extends EntityBase, TDto extends DtoBase>{
  ...
  async create(entity: RequestModel<TDto>): Promise<ResponseModel<TDto>> {
    try {
      await console.log("\nInside insert of generic repository...entity is...." + JSON.stringify(entity));
      var result: TDto[] = [];
      ...
      */
      ...
      console.log("\nError occured while inserting entity....." + error);
  ...
  //***************** Fetches records from database using a unique id passed as a argument **********************************//
  ...
  async getByTickerSymbol(table: string, ticker_symbol: string): Promise<ResponseModel<TDto>> {
    try {
      ...
      final_result.setDataCollection(await this.mapToDto(await this.genericRepository
        .createQueryBuilder(table).where("ticker_symbol= :ticker_symbol", {ticker_symbol}).select("*").execute()))
      return final_result;
    }
    catch (error) {
      console.log("Error is....." + error);
  ...
  async getUserBySub(sub: string): Promise<ResponseModel<TDto>> {
    try {
      ...
      console.log(sub);
      final_result.setDataCollection(await this.mapToDto(await this.genericRepository
        .createQueryBuilder("user").where("sub= :sub", {sub}).select("*").execute()))
      return final_result;
    }
    catch (error) {
      console.log("Error is....." + error);
  ...
  async getByUserId(table: string, id: number): Promise<ResponseModel<TDto>> {
    try {
      ...
      console.log(id);
      final_result.setDataCollection(await this.mapToDto(await this.genericRepository
        .createQueryBuilder(table).where("user_id=:id", {id}).select("*").execute()))
      return final_result;
    }
    catch (error) {
      console.log("Error is....." + error);
  ...
  async getSpecificOrderByUserId(ticker_symbol: string, id: number): Promise<ResponseModel<TDto>> {
    try {
      ...
      console.log(id);
      final_result.setDataCollection(await this.mapToDto(await this.genericRepository
        .createQueryBuilder("order").where("user_id=:id and ticker_symbol=:ticker_symbol", {id, ticker_symbol}).select("*").execute()))
      return final_result;
    }
    catch (error) {
      console.log("Error is....." + error);

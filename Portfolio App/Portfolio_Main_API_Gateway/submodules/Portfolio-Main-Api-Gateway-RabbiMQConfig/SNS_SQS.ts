...
let rabbitMQURL = 'amqps://hzvbyxps:x4m-DVhhEPRJFXTZ08Nc7Piz9KeWS_-k@puffin.rmq2.cloudamqp.com/hzvbyxps';
...
export class SNS_SQS {
  ...
  public publishPlatformMessageToTopics(
    serviceName: string,  //PRODUCT_SERVICE
    topicName: string,  //PRODUCT_ADD
    methodName: string,
    message: any,
    ...
    this.publishPlatformMessageToTopic(topicName, message,id);
  }

  public async publishPlatformMessageToTopic(topicName, message,id) {
    // publish the message to topic
    let jsondata = JSON.parse(message);
    console.log("json data: ", jsondata)
    jsondata.DataCollection[0].Id = id;
